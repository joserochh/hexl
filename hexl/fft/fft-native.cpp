...
void Fwd_FFT(std::complex<double>* result, const std::complex<double>* operand,
             ...
             const uint64_t n, const double* scalar) {
  HEXL_CHECK(inv_root_of_unity_powers != nullptr,
             "inv_root_of_unity_powers == nullptr");
  HEXL_CHECK(operand != nullptr, "operand == nullptr");
  HEXL_CHECK(result != nullptr, "result == nullptr");
...
void Inv_FFT(std::complex<double>* result, const std::complex<double>* operand,
  ...
  std::complex<double> roots[n];
  std::complex<double> root_of_unity_powers[n];
  const std::complex<double> J(0, 1);
  size_t offset = 0;
  for (; m > 0; m >>= 1) {
    std::complex<double> wO(1, 0);
    std::complex<double> wmO = std::exp(J * ((PI_ * -1) / gap));
    for (int j = 0; j < gap; ++j) {
      for (int k = 0; k < m; k++) {
        root_of_unity_powers[root_index] = wO;
        std::cout << root_index << " " << wO << std::endl;
  ...
  for (size_t i = 0; i < n; ++i) {
    result[i] = result[i] / (static_cast<double>(n));
  }
}
